...
# missing-column error in whichever migration ran too early.
	...
	@psql -v ON_ERROR_STOP=1 -q -v runtime_password=$(TEST_RUNTIME_PASSWORD) -v provider_password=$(TEST_PROVIDER_PASSWORD) -v dispatch_password=$(TEST_DISPATCH_PASSWORD) -v resolution_password=$(TEST_RESOLUTION_PASSWORD) -f scripts/ci-fixture.sql -d "$(CI_DSN)"
