...
| 6 | ☑ Multi-consumer delivery substrate: **closed by decision, not built** | Single-consumer delivery was chosen deliberately: no substrate upgrade without a proven need (RESPONSE-14). No second consumer exists, and delivering one event to several consumers with separate outcomes would change `platform.dead_letter`'s key to (event, consumer), the dispatcher's claim, and every replay and resolve path. Item 5 already stops one consumer's debt refusing another. The single-active-consumer gate stays, with `Retire` as its exit. **Reopen when** a second projection consumer is named | RESPONSE-14, RESPONSE-15, RESPONSE-23 |
...
| 19 | The consumer cannot apply `projection.repair.reconciled` | Reconciliation publishes a repair event whose findings carry only versions, not the state to apply, so foundation-reference refuses it as poison and it dead-letters. It is not an authority type, so it blocks nothing, but every repair the reconciler publishes becomes an incident. Either the finding carries the Membership or Tenant state it repairs, or the consumer answers a repair by fetching that state. Spans organization-control and foundation-reference | found while building item 16 |
